(function() {
    ...
    const AppState = {
        ...
        createDefaultPortfolio() {
            ...
            if (this.marketData?.stocks) {
                ...
                for (let i = 29; i >= 0; i--) {
                    ...
                    const randomChange = (Math.random() * 0.1 - 0.02) * STARTING_BALANCE;
                    portfolio.history.push({
                        date,
                        value: STARTING_BALANCE + randomChange
                    });
                }
            }
        ...
        async updateLeaderboard() {
            ...
            const userReturn = ((portfolioValue - STARTING_BALANCE) / STARTING_BALANCE) * 100;

            // Update or add user to leaderboard
            const userIndex = this.leaderboard.findIndex(e => e.name === (this.user?.name || 'You'));
            const userEntry = {
        ...
        calculatePortfolioValue() {
            if (!this.portfolio || !this.marketData) return STARTING_BALANCE;
        ...
        calculateMonthlyPerformance() {
            ...
            const thirtyDaysAgo = this.portfolio.history?.[0]?.value || STARTING_BALANCE;
            return ((currentValue - thirtyDaysAgo) / thirtyDaysAgo) * 100;
        },
    ...
    const UI = {
        ...
        setupMobileMenu() {
            ...
            mobileContainer.innerHTML = `
                <div class="px-4 py-2 space-y-2">
                    ...
                    <a href="practice.html" class="block py-3 px-4 text-green-600 font-medium hover:bg-green-50 rounded-lg transition focus:outline-none focus:ring-2 focus:ring-green-500">Practice</a>
        ...
        showNotifications() {
            ...
            modal.innerHTML = `
                <div class="bg-white rounded-xl max-w-lg w-full mx-4 p-6 max-h-[80vh] overflow-y-auto">
                    ...
                    <div class="space-y-3">
                        ${notifications.map(n => `
                            <div class="p-4 bg-green-50 rounded-lg animate-fade-in">
                                <div class="flex items-start">
        ...
        renderHoldings() {
            ...
            if (Object.keys(holdings).length === 0) {
                container.innerHTML = `
                    <div class="text-center py-8 px-4">
                        ...
                        <p class="text-gray-600">No holdings yet. Start trading!</p>
                    </div>
                `;
                return;
            }
            ...
            container.innerHTML = Object.entries(holdings).map(([symbol, holding]) => {
                ...
                const percentOfPortfolio = totalValue ? (value / (AppState.portfolio.cash + totalValue)) * 100 : 0;

                return `
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-gray-50 rounded-lg hover:shadow-md transition cursor-pointer holding-item" 
                         data-symbol="${symbol}"
                         tabindex="0"
                         role="button">
                        <div class="flex items-center mb-2 sm:mb-0">
                            <div class="w-10 h-10 ${this.getSectorColor(stock.sector)} rounded-lg flex items-center justify-center mr-3 flex-shrink-0">
                                <span class="font-bold text-white text-sm">${symbol}</span>
                            </div>
                            <div class="min-w-0 flex-1">
                                <h4 class="font-medium text-gray-800 truncate">${stock.name}</h4>
                        ...
                        <div class="flex justify-between sm:block ml-13 sm:ml-0">
        ...
        updateMarketData() {
            ...
            tableContainer.innerHTML = `
                <h3 class="text-lg font-semibold text-gray-800 mb-4 px-4">Market Watch</h3>
                                ...
                                <tr class="border-b hover:bg-gray-50 transition">
                                    ...
                                    <td class="py-3 px-4">
                                        <div class="flex flex-col sm:flex-row gap-2">
                                            <button class="bg-green-500 hover:bg-green-600 text-white px-3 py-2 rounded text-sm transition focus:outline-none focus:ring-2 focus:ring-green-500 min-h-[44px] buy-btn" data-symbol="${stock.symbol}">Buy</button>
                                            <button class="border border-green-500 text-green-600 hover:bg-green-50 px-3 py-2 rounded text-sm transition focus:outline-none focus:ring-2 focus:ring-green-500 min-h-[44px] sell-btn" data-symbol="${stock.symbol}">Sell</button>
                                        </div>
                                    </td>
                                </tr>
                            `).join('')}
        ...
        openTradeModal(stock, action) {
            ...
            modal.setAttribute('role', 'dialog');
            modal.setAttribute('aria-modal', 'true');
            modal.setAttribute('aria-labelledby', 'trade-title');
            ...
            modal.innerHTML = `
                <div class="bg-white rounded-xl max-w-md w-full mx-4 p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h3 id="trade-title" class="text-xl font-bold">${action === 'buy' ? 'Buy' : 'Sell'} ${stock.symbol}</h3>
                        <button class="text-gray-500 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-500 rounded-lg p-2" id="close-modal" aria-label="Close">
                            <i class="fas fa-times text-xl"></i>
                        </button>
                    </div>
                    ...
                    <div class="mb-4 space-y-3">
                        ...
                        <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                            ...
                            <span class="font-bold text-green-600">Ksh ${currentPrice.toFixed(2)}</span>
                        </div>
                        <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <div class="mb-4">
                        <label class="block text-gray-700 font-medium mb-2" for="share-quantity">Number of Shares</label>
                        <input type="number" id="share-quantity" 
                    ...
                    <div class="bg-green-50 rounded-lg p-4 mb-6">
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-gray-700">Total ${action === 'buy' ? 'Cost' : 'Proceeds'}:</span>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <button class="flex-1 px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition focus:outline-none focus:ring-2 focus:ring-gray-500 min-h-[44px]" id="cancel-trade">
                            Cancel
                        </button>
                        <button class="flex-1 px-6 py-3 ${action === 'buy' ? 'bg-green-500 hover:bg-green-600' : 'bg-red-500 hover:bg-red-600'} text-white rounded-lg transition focus:outline-none focus:ring-2 focus:ring-offset-2 min-h-[44px]" id="execute-trade">
                            ${action === 'buy' ? 'Buy' : 'Sell'}
                        </button>
                    </div>
                </div>
        ...
        updateLeaderboard() {
            ...
            const userReturn = ((userPortfolioValue - STARTING_BALANCE) / STARTING_BALANCE) * 100;
                ...
                html += `
                    <div class="flex items-center justify-between p-3 ${isUser ? 'bg-blue-50' : index === 0 ? 'bg-yellow-50' : 'bg-gray-50'} rounded-lg">
                        <div class="flex items-center min-w-0 flex-1">
                            <span class="w-6 h-6 ${rankColors[index]} text-white rounded-full flex items-center justify-center text-sm font-bold mr-3 flex-shrink-0">${entry.rank}</span>
                            <div class="min-w-0">
                                <h4 class="font-semibold text-gray-800 truncate">${entry.name} ${isUser ? '(You)' : ''}</h4>
                ...
                leaderboard.slice(3).forEach(entry => {
                    ...
                    html += `
                        <div class="flex items-center justify-between p-3 ${isUser ? 'bg-blue-50' : 'bg-gray-50'} rounded-lg">
                            <div class="flex items-center min-w-0 flex-1">
                                <span class="w-6 h-6 bg-gray-300 text-white rounded-full flex items-center justify-center text-sm font-bold mr-3 flex-shrink-0">${entry.rank}</span>
                                <div class="min-w-0">
                                    <h4 class="font-semibold text-gray-800 truncate">${entry.name} ${isUser ? '(You)' : ''}</h4>
                                    <p class="text-gray-600 text-sm">${this.formatCurrency(entry.portfolio)}</p>
                                </div>
                            </div>
                            <span class="font-semibold ${entry.return >= 0 ? 'text-green-600' : 'text-red-600'} ml-2">
                                ${entry.return >= 0 ? '+' : ''}${entry.return.toFixed(1)}%
                            </span>
                        </div>
                    `;
            ...
            if (viewAllBtn) {
                ...
                viewAllBtn.classList.add('min-h-[44px]', 'px-4', 'py-2');
            }
        },
            ...
            container.innerHTML = `
                ...
                <div class="flex justify-between items-center p-3 hover:bg-gray-50 rounded-lg transition">
                    <div>
                        ...
                        <p class="text-xl font-bold ${bestReturn >= 0 ? 'text-green-600' : 'text-red-600'}">
        ...
        calculateBestReturn() {
            ...
            return ((currentValue - STARTING_BALANCE) / STARTING_BALANCE) * 100;
        },
        ...
        setupLoanSimulator() {
            ...
            loanBtn.classList.add('min-h-[44px]');
        },
            ...
            modal.innerHTML = `
                <div class="bg-white rounded-xl max-w-lg w-full mx-4 p-6 max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="number" id="loan-term" class="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" 
                               value="12" min="3" max="60">
                    </div>
                    
                    <button class="w-full bg-indigo-500 hover:bg-indigo-600 text-white py-3 rounded-lg font-semibold transition focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 min-h-[44px] mb-6" id="calculate-loan">
                        Calculate Options
                    </button>
                    
                    <div id="loan-results" class="hidden space-y-4"></div>
                </div>
            `;
        ...
        calculateLoanOptions(amount, months) {
            const options = [
                { name: 'M-Shwari', rate: 0.12, color: 'text-red-600', bgColor: 'bg-red-50' },
                { name: 'Bank Loan', rate: 0.15, color: 'text-yellow-600', bgColor: 'bg-yellow-50' },
                { name: 'SACCO', rate: 0.10, color: 'text-green-600', bgColor: 'bg-green-50' }
            ];

            const resultsDiv = document.getElementById('loan-results');
            let html = '<h4 class="font-bold mb-3">Comparison Results</h4>';
            ...
            options.forEach(option => {
                ...
                html += `
                    <div class="p-4 ${option.bgColor} rounded-lg">
                        <div class="flex justify-between items-center mb-2">
        ...
        setupInsuranceSimulator() {
            ...
            insuranceBtn.classList.add('min-h-[44px]');
        },
            ...
            modal.innerHTML = `
                <div class="bg-white rounded-xl max-w-lg w-full mx-4 p-6">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="claim-type" class="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" id="claim-amount" class="w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" 
                               value="5000" min="1000" max="50000" step="500">
                    </div>
                    
                    <div class="mb-6">
                        <label class="block text-gray-700 font-medium mb-2">Upload Document</label>
                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-green-500 transition" id="upload-area" role="button" tabindex="0">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <button class="flex-1 px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition focus:outline-none focus:ring-2 focus:ring-gray-500 min-h-[44px]" id="cancel-claim">
                            Cancel
                        </button>
                        <button class="flex-1 px-6 py-3 bg-cyan-500 text-white rounded-lg hover:bg-cyan-600 transition focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 min-h-[44px]" id="process-claim">
                            Process Claim
                        </button>
                    </div>
        ...
        processInsuranceClaim(type, amount) {
            ...
            if (success) {
                ...
                resultDiv.className = 'mt-6 p-4 bg-green-50 rounded-lg animate-fade-in';
                resultDiv.innerHTML = `
                    <div class="text-center">
                        ...
                        <p class="text-green-600 font-bold text-lg">${this.formatCurrency(payout)}</p>
                    </div>
                `;
            } else {
                resultDiv.className = 'mt-6 p-4 bg-red-50 rounded-lg animate-fade-in';
                resultDiv.innerHTML = `
                    <div class="text-center">
                        ...
                        <p class="text-gray-600">Insufficient documentation. Please review requirements.</p>
                    </div>
                `;
            }
        ...
        setupBudgetChallenge() {
            ...
            joinBtn.classList.add('min-h-[44px]');
        },

        joinBudgetChallenge() {
            const challengeData = localStorage.getItem(STORAGE_KEYS.CHALLENGE + AppState.user?.userId);
            ...
            if (challengeData) {
                ...
                localStorage.setItem(STORAGE_KEYS.CHALLENGE + AppState.user?.userId, JSON.stringify(newChallenge));
        ...
        showChallengeModal(challenge) {
            ...
            modal.innerHTML = `
                <div class="bg-white rounded-xl max-w-lg w-full mx-4 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-6">
                        <button class="p-4 bg-green-50 rounded-lg hover:bg-green-100 transition focus:outline-none focus:ring-2 focus:ring-green-500 min-h-[44px] spending-btn" data-category="food" data-amount="200">
                            <i class="fas fa-utensils text-green-600 text-xl mb-2"></i>
                            <p class="font-medium">Food</p>
                            <p class="text-sm text-gray-600">Ksh 200</p>
                        </button>
                        <button class="p-4 bg-blue-50 rounded-lg hover:bg-blue-100 transition focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[44px] spending-btn" data-category="transport" data-amount="150">
                            <i class="fas fa-bus text-blue-600 text-xl mb-2"></i>
                            <p class="font-medium">Transport</p>
                            <p class="text-sm text-gray-600">Ksh 150</p>
                        </button>
                        <button class="p-4 bg-purple-50 rounded-lg hover:bg-purple-100 transition focus:outline-none focus:ring-2 focus:ring-purple-500 min-h-[44px] spending-btn" data-category="entertainment" data-amount="300">
                        ...
                        <button class="p-4 bg-orange-50 rounded-lg hover:bg-orange-100 transition focus:outline-none focus:ring-2 focus:ring-orange-500 min-h-[44px] spending-btn" data-category="other" data-amount="100">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <button class="flex-1 px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition focus:outline-none focus:ring-2 focus:ring-gray-500 min-h-[44px]" id="end-day">
                            End Day
                        </button>
                        <button class="flex-1 px-6 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600 transition focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 min-h-[44px]" id="reset-challenge">
                            Reset
                        </button>
                    </div>
                </div>
            ...
            document.querySelectorAll('.spending-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    if (remaining - amount >= 0) {
                        ...
                        localStorage.setItem(STORAGE_KEYS.CHALLENGE + AppState.user?.userId, JSON.stringify(challenge));
            ...
            document.getElementById('end-day').addEventListener('click', () => {
                if (challenge.day >= 30) {
                    ...
                    localStorage.removeItem(STORAGE_KEYS.CHALLENGE + AppState.user?.userId);
                    modal.remove();
                } else {
                    challenge.day++;
                    challenge.spent = 0;
        ...
        async initialize() {
            ...
            this.updateLeaderboard();
            this.updatePracticeStats();
            this.setupLoanSimulator();
            this.setupInsuranceSimulator();
            this.setupBudgetChallenge();
            ...
            announcement.textContent = 'Practice zone loaded. You can start trading with virtual money.';
            document.body.appendChild(announcement);
            setTimeout(() => announcement.remove(), 3000);

            console.log('Practice zone initialized');
        }
    };
    ...
    style.textContent = `
        ...
        @media (max-width: 640px) {
            ...
            button, a, .holding-item, [role="button"] {
                min-height: 44px;
            }
            input, select {
